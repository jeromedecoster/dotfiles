# z.sh special edition for my dotfiles
# all commented lines are marked '# DOTFILES COMMENTED'
# all added lines are marked '# DOTFILES ADDED'
# this is based on the file
# https://github.com/rupa/z/commits/master/z.sh
# from rev: Jun 23, 2013
...
_z() {
    ...
    if [ "$1" = "--add" ]; then
        ...
        while read line; do
            ...
            [ -d "${line%%\|*}" ] && echo $line
        # done < "$datafile" | awk -v path="$*" -v now="$(date +%s)" -F"|" '    # DOTFILES COMMENTED, below ADDED
        done < "$datafile" | awk -v path="$path" -v now="$(date +%s)" -F"|" '
            BEGIN {
                rank[path] = 1
                time[path] = now
            }
    ...
    # tab completion
        ...
        case "$last" in
            ...
            # /*) [ -z "$list" -a -d "$last" ] && cd "$last" && return;;    # DOTFILES COMMENTED

            # DOTFILES ADDED : jump to a directory after a bash completion (the full path is
            # expanded by the completion system. This jump increase the rank of the jumped folder
            /*) [ -z "$list" -a -d "$last" ] && cd "$last" && $(z --add "$last") && return;;
        esac

        # no file yet
        # [ -f "$datafile" ] || return   # DOTFILES COMMENTED
        [ -f "$datafile" ] || return 0   # DOTFILES ADDED
        ...
        ')"
        [ $? -gt 0 ] && return
        # [ "$cd" ] && cd "$cd"    # DOTFILES COMMENTED

        # DOTFILES ADDED : jump to a directory found by a part of the name (z find
        # the best match). This jump increase the rank of the jumped folder
        [ "$cd" ] && cd "$cd" && $(z --add "$cd")
        
        return 0    # DOTFILES ADDED prevent exitcode 1
    fi
}
